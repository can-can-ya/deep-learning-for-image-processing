...
def plot_loss_and_lr(train_loss, learning_rate, file_name):
    try:
        ...
        fig.savefig('./' + file_name + '_loss_and_lr{}.png'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))
        plt.close()
...
def plot_map(mAP, file_name):
    try:
        ...
        plt.xlim(0, len(mAP))
        plt.legend(loc='best')
        plt.savefig('./' + file_name + '_mAP.png')
        plt.close()
